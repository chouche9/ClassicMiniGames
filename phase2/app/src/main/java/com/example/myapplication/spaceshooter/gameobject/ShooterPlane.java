...
public class ShooterPlane extends ShooterGameObject implements Parcelable {
  ...
  public ShooterPlane(Context context, int planeStyle) {
    ...
    setObject(Bitmap.createScaledBitmap(getObject(), 300, 200, false));
    setX(dWidth / 2 - 150);
    setY(dHeight - 300);
    setWidthHeight();
  }
  ...
  public static final Creator<ShooterPlane> CREATOR =
      new Creator<ShooterPlane>() {
        @Override
  ...
  public boolean touchInRange(float touchX, float touchY) {
    return getX() <= touchX
        && touchX <= getX() + getWidth()
        && getY() <= touchY
        && touchY <= getY() + getHeight();
  }
