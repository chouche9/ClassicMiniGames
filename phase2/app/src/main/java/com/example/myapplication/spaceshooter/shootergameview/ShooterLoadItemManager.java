...
class ShooterLoadItemManager {
  ...
  private void updateEnemy() {
    ...
    for (ShooterEnemy enemy1 : enemies) {
      enemy1.setY(enemy1.getY() + enemy1.getVelocity());
  ...
  private void updateEnemybullet() {
    ...
    if (enemyBulletCount == 14) {
      for (ShooterEnemy enemy1 : enemies) {
        if (enemy1.getY() > 0) {
          shooterEnemyBullets.add(
              new ShooterEnemyBullet(
                  context,
  ...
  private void updatePlanebullet() {
    ...
    if (planeBulletCount == 5) {
      shooterPlaneBullets.add(
          new ShooterPlaneBullet(context, plane.getX() + plane.getWidth() / 2, plane.getY()));
